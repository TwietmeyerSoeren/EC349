...
## use hexbin to plot correlation between different columns
...
text_test_df <- review_data_small[1:200000, ]
## prepare text for analysis:
corpus <- Corpus(VectorSource(text_test_df$text))
...
text_test_df$text_cleaned <- sapply(corpus, as.character)

## business data
cols_keep_business_data <- empty_rows_business_data %>% dplyr::filter(`EmptyRows` < 20000 & !Column %in% c('name', 'address', 'latitude', 'longitude'))

## tip data:
num_comments_by_business <- tip_data %>% group_by(business_id) %>% summarise(total_comments_business = n())

# user data:
user_data_selected <- user_data_small  %>% mutate(yelping_since_weeks = round(as.numeric(difftime("2023-12-31 00:00:00", yelping_since, units = "weeks")), digits = 0), num_friends = ifelse(friends != "None", sapply(strsplit(friends, ","), function(x) length(x)), 0), total_compliments = select(., starts_with("compliment_")) %>% rowSums(na.rm = TRUE)) %>% select(-c(name, yelping_since))

# merge other data sets onto review data which is the main one:
review_data_combined <- review_data_small %>% left_join(business_data[, cols_keep_business_data$Column], by = 'business_id', suffix = c('_review', '_business')) %>% left_join(user_data_selected, by = 'user_id', suffix = c('_review', '_user')) %>% left_join(num_comments_by_business, by = 'business_id')
view(count_empty_rows(review_data_combined))

```
### Data Structure
1. What data structure is required for our prediction?
- one test and one training dataset
- test has 10,000 entries
- how do we deal with multiple reviews?
- what is the structure of the data? i.e. based on individual i and business j?

2. What can be added from which datasets?
- main dataset: review data small
- additions from other datasets: 
2.1 average rating of business (in business data), average rating customer gives (in user_data_small)
2.2 check in data: number of checkins made on a business
2.3 review data/ tip data: number of reviews/ comments given to a business, quality of tips given to a business by text content + weigthed by reactions
2.4 business data: all cols without too many missing values
- is using means even allowed? what does the test data set need to look like? can we add new information to that too?
2.5 user data: num friends, yelping since in month, useful, funny, cool, elite, fans, average_stars, maybe compliment data
...
final_dataset <- review_data_combined %>% select(-c("date"))
...
lm_stars<- lm(stars_review ~ user_id, data = data_train)

#Review the results
summary(lm_stars)

#Prediction to test data
lm_stars_predict<-predict(lm_cars, newdata = data_test[,-c("stars_review")])
...
## LASSO and Ridge
shrinkage_estimator_computation <- function(lasso) {
    cv_out <- cv.glmnet(as.matrix(data_train_x), as.matrix(data_train_y), alpha = lasso, nfolds = 3) # adapt the number of folds
    cv_out_plot <- plot(cv_out)
    lambda_cv <- cv_out$lambda.min

    #Re-Estimate Ridge with lambda chosen by Cross validation
    model<-glmnet(data_train_x, data_train_y, alpha = lasso, lambda = lambda_cv, thresh = 1e-12) # what does the thresh mean?

    #Fit on Test Data
    predictions <- predict(ridge_model, s = lambda_ridge_cv, newx = as.matrix(data_test_x))
    mse <- mean((ridge_predictions - data_test_y) ^ 2)
...
cv_out_ridge <- cv.glmnet(as.matrix(data_train_x), as.matrix(data_train_y), alpha = 0, nfolds = 3) # adapt the number of folds
plot(cv_out_ridge)
lambda_ridge_cv <- cv_out_ridge$lambda.min

#Re-Estimate Ridge with lambda chosen by Cross validation
ridge_model<-glmnet(data_train_x, data_train_y, alpha = 0, lambda = lambda_ridge_cv, thresh = 1e-12) # what does the thresh mean?

#Fit on Test Data
ridge_predictions <- predict(ridge_model, s = lambda_ridge_cv, newx = as.matrix(data_test_x))
ridge_mse <- mean((ridge_predictions - data_test_y) ^ 2)

#LASSO with Cross-Validation
cv_out_lasso <- cv.glmnet(as.matrix(data_train_x), as.matrix(data_train_y), alpha = 1, nfolds = 3) # adapt the number of folds
plot(cv_out_lasso)
lambda_LASSO_cv <- cv_out_lasso$lambda.min

#Re-Estimate LASSO with lambda chosen by Cross validation
lasso_model <- glmnet(data_train_x, data_train_y, alpha = 1, lambda = lambda_LASSO_cv, thresh = 1e-12)
coef(lasso_model) #note that some parameter estimates are set to 0 --> Model selection!

#Fit on Test Data
lasso_predictions <- predict(lasso_model, s = lambda_LASSO_cv, newx = as.matrix(data_test_x))
lasso_mse <- mean((lasso_predictions - data_test_y) ^ 2) #Note how it outperforms OLS
...
columns_to_process <- c("belli")
adapted_df <- final_dataset %>% mutate_at(vars(columns_to_process), funs(ifelse(. > 0, 1, 0)))
...
cv_out_ridge_binary <- cv.glmnet(as.matrix(data_train_binary_x), as.matrix(data_train_binary_y), alpha = 0, nfolds = 3) # adapt the number of folds
plot(cv_out_ridge_binary)
lambda_ridge_cv_binary <- cv_out_ridge_binary$lambda.min

#Re-Estimate Ridge with lambda chosen by Cross validation
ridge_model_binary<-glmnet(data_train_binary_x, data_train_binary_y, alpha = 0, lambda = lambda_ridge_cv_binary, thresh = 1e-12) # what does the thresh mean?

#Fit on Test Data
ridge_predictions_binary <- predict(ridge_model_binary, s = lambda_ridge_cv_binary, newx = as.matrix(data_test_binary_x))
ridge_mse_binary_binary <- mean((ridge_predictions_binary - data_test_binary_y) ^ 2)

#LASSO with Cross-Validation
cv_out_lasso_binary <- cv.glmnet(as.matrix(data_train_binary_x), as.matrix(data_train_binary_y), alpha = 1, nfolds = 3) # adapt the number of folds
plot(cv_out_lasso_binary)
lambda_LASSO_cv_binary <- cv_out_lasso_binary$lambda.min

#Re-Estimate LASSO with lambda chosen by Cross validation
lasso_model_binary <- glmnet(data_train_binary_x, data_train_binary_y, alpha = 1, lambda = lambda_LASSO_cv_binary, thresh = 1e-12)
coef(lasso_model_binary) #note that some parameter estimates are set to 0 --> Model selection!

#Fit on Test Data
lasso_predictions_binary <- predict(lasso_model_binary, s = lambda_LASSO_cv_binary, newx = as.matrix(data_test_binary_x))
lasso_mse_binary <- mean((lasso_predictions_binary - data_test_binary_y) ^ 2) #Note how it outperforms OLS
