...
Whilst the user data set contains valuable information that show significant correlation with our variable of interest such as the number of weeks since a user joined yelp, there are only 20% of the users in the review data set present in the user data set. This large number of missing users does not allow for inclusion, given the inability to construct meaningful averages to fill the data or the ignorance of the missing data points.
...
Nevertheless, boosting is able to achieve a performance comparable to the shrinkage estimators. This is due to the combination of bagging and learning. Bagging reduces the variance of the estimates by using averages and by letting the estimator explain the previously unexplained data points, a precise estimate can be achieved.
