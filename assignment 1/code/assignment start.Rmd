...
```{r business review plot, echo=FALSE, warning=FALSE, fig.cap="Graph showing the strong positive correlation between business ratings and user reviews.", fig.align='center', fig.id=TRUE}
business_review_comp
```

```{r state plot, echo=FALSE, warning=FALSE, fig.cap="Graph showing differences in user ratings between different states.", fig.align='center', fig.id=TRUE}
state_plot
```

The tip data only includes additional comments made from a user to a business and can be omitted given the inclusion of the comments of the reviews from the review data set. The check-in data contains the times a user looked at a particular business. After aggregating the data, a slight negative non-linear relationship can be seen in the data. Thus it can be used as a feature.
Whilst the user data set contains valuable information that show significant correlation with our variable of interest such as the number of weeks since a user joined yelp, there are only 20% of the users in the review data set present in the user data set. Therefore, we are unable to construct meaningful averages to fill the data or ignore the missing data points forcing us to omit the data entirely.

```{r yelping since plot, echo = FALSE, warning=FALSE, fig.cap="Graph showing the slight positive relationship between the length of usership and user reviews.", fig.align='center', fig.id=TRUE}
...
# for ridge, only 65 features have a coefficient larger than 0.1 view(coefficients_ridge %>% filter(coefficient_abs_value > 0.1))
...
cv_out_ridge_binary <- ridge_results_binary$cv_out
plot_cv_out_ridge_binary <- ridge_results_binary$cv_out_plot
lambda_ridge_cv_binary <- ridge_results_binary$lambda_cv
ridge_model_binary <- ridge_results_binary$model
ridge_mse_binary <- ridge_results_binary$mse
ridge_mse_training_binary <- ridge_results_binary$mse_training
ridge_rsq_binary <- ridge_results_binary$rsq
ridge_rsq_training_binary <- ridge_results$rsq_training
...
model_performance_xgboost <- data_test %>%
 ...
 metrics(truth = stars_review, estimate = predictions)
```
Our data set contains a large number of parameters as well as a large number of observations. However, only few are likely to be highly correlated with our variable of interest. Variables such as the average number of stars given to a business are likely to by highly correlated with the number of stars of a review however, individual words taken from the review will only reveal low correlation. In these cases, models performing variable selection/ shrinking will do better since they are able to largely reduce the variance which emergence due to the presence of many parameters. The OLS regression can neither drop nor shrink the coefficient of certain features and is hence plagued by this high variance. Ridge and LASSO models both benefit from the ability to shrink coefficients to pick the most predictive ones hence reducing the negative impact of variables of low explanatory power and high multicolinearity.

### OLS regression
Since we are not concerned with endogeneity, we can select various models to find the model which performs the best. However, including a large number of parameters will lead to poor performance given the higher variance. The model $stars\_review_{ij} =   cool\_review_i+ cool\_review_i^2+ cool\_review_i^3 + review\_count_i + funny_i + funny_i^2 + funny_i^3 + useful_i + useful_i^2 + useful_i^3 + stars\_business_j + total\_comments\_business_j  + state_j + is\_open_j + num\_checkins\_business_j + num\_checkins\_business_j^2 + num\_checkins\_business_j^3 + \epsilon_i$ performed the best on the test data set with a MSE of 1.54. Through including the squared and cubic terms, we can achieve a lower MSE since the relationship is, as seen in the data exploration phase, non-linear. Some features have been omitted since they did not lead to increases in model performance. The high variance of the OLS regression, caused by the adherence to the low bias, leads to a relatively poor performance. 

### Ridge and LASSO
As mentioned above, LASSO and Ridge allow us to include more parameters without significantly increasing the variance. Despite a small increase in bias, the large decrease in variance achieved through shrinking the coefficients has led to a significantly better performance, reaching a MSE of XXX using Ridge and a MSE of XXX using LASSO. Additionally, the text is unlikely to be related in a highly non-linear way, wherefore LASSO and Ridge both perform well. The remaining variables are potentially related in a non-linear way, however the failure to capture these relationships attenuated by the inclusion of the text. 
...
As mentioned above, the shrinkage estimators have a significant advantage over the OLS estimator due to the ability to either shrink the coefficient or perform variable selection. This advantage is reflected in a significantly lower MSE. 
...
Regression trees are unlikely to perform well given the large number of parameters and the low levels of informativeness of each feature. Under these circumstances, a large tree would be required, leading to a high variance and poor performance. Using Random Forests will lead to poor performance due to the large number of parameters and few good predictors. 
Nevertheless, boosting is able to achieve a performance comparable to the shrinkage estimators. This is due to the combination of bagging and learning ad the ability to capture non-linear relationships between the variables. Bagging reduces the variance of the estimates by using averages and by letting the estimator explain the previously unexplained data points, a precise estimate can be achieved.

## Project Challenges
A major challenge during the project was to determine what is required of the data, how the data can be transformed to be made useful and how the ultimate goal of prediction can best be reached. However, using an iterative approach of exploring the data, assessing its usefulness whilst referring to the initial goal has proven itself particularly instructive. Applying the same strategy to the modelling phase was equally effective.
...
Related articles (https://eur01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwarwick.ac.uk%2Fservices%2Fgov%2Fcalendar%2Fsection2%2Fregulations%2Facademic_integrity%2F&data=05%7C01%7CSoeren.Twietmeyer%40warwick.ac.uk%7C712480e0fa474f30a32208dbf193083d%7C09bacfbd47ef446592653546f2eaf6bc%7C0%7C0%7C638369386071528461%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C3000%7C%7C%7C&sdata=LvxJFDdmxw35SUoFzS0TkpCD7X9LEpairvIN6Jc13bY%3D&reserved=0)

Reg. 11 Academic Integrity (from 4 Oct 2021) (https://eur01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwarwick.ac.uk%2Fservices%2Faro%2Fdar%2Fquality%2Faz%2Facintegrity%2Fframework%2Fguidancereg11%2F&data=05%7C01%7CSoeren.Twietmeyer%40warwick.ac.uk%7C712480e0fa474f30a32208dbf193083d%7C09bacfbd47ef446592653546f2eaf6bc%7C0%7C0%7C638369386071537704%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C3000%7C%7C%7C&sdata=6LkwxuBqkfuytAHv8NbckOYEEU5xGvdbJaQO%2BrNHZDw%3D&reserved=0)

Guidance on Regulation 11 (https://eur01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwarwick.ac.uk%2Fservices%2Faro%2Fdar%2Fquality%2Fcategories%2Fexaminations%2Fpolicies%2Fv_proofreading%2F&data=05%7C01%7CSoeren.Twietmeyer%40warwick.ac.uk%7C712480e0fa474f30a32208dbf193083d%7C09bacfbd47ef446592653546f2eaf6bc%7C0%7C0%7C638369386071544268%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C3000%7C%7C%7C&sdata=lZU%2B81ge6eDnBDczFj3wOzVlyUKD%2FsUUkr8o%2FR7NAuw%3D&reserved=0)

Proofreading Policy (https://eur01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwarwick.ac.uk%2Fservices%2Faro%2Fdar%2Fquality%2Faz%2Facintegrity%2Fframework%2Fguidancereg11%2F&data=05%7C01%7CSoeren.Twietmeyer%40warwick.ac.uk%7C712480e0fa474f30a32208dbf193083d%7C09bacfbd47ef446592653546f2eaf6bc%7C0%7C0%7C638369386071550863%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C3000%7C%7C%7C&sdata=3MpWklCOg0ghrmY1TLdjSNSYr9oTSyDShMN9A4UJRrc%3D&reserved=0)
...
Academic Integrity (warwick.ac.uk) (https://eur01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwarwick.ac.uk%2Fstudents%2Flearning-experience%2Facademic_integrity&data=05%7C01%7CSoeren.Twietmeyer%40warwick.ac.uk%7C712480e0fa474f30a32208dbf193083d%7C09bacfbd47ef446592653546f2eaf6bc%7C0%7C0%7C638369386071557453%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C3000%7C%7C%7C&sdata=RjQCj2upSBuExQpXwhDvkPDvhw1wvFg%2BRzpGXjlbceQ%3D&reserved=0)