...
model_performance <- data_test %>%
 ...
 metrics(truth = stars_review, estimate = predictions)
# gets a performance of 0.909 on the test data
```
...
Boosting is able to achieve a performance comparable to the shrinkage estimators. This is due to the combination of bagging and learning. Bagging reduces the variance of the estimates by using averages and by letting the estimator explain the previously unexplained data points, a precise estimate can be achieved i.e. the ability to fit the previously unexplained parts of the data. 
